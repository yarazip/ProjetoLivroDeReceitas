...
if (isset($_GET['excluir'])) {
    ...
    try {
        ...
        $conn->commit();
        header("Location: receitasChef.php?success=2"); // Código 2 para exclusão
        exit();
    } catch (Exception $e) {
        $conn->rollBack();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (isset($_POST['add']) || isset($_POST['editar']))) {
    ...
    try {
        ...
        if ($edicaoAtiva && isset($_POST['editar'])) {
            ...
            $conn->commit();
            header("Location: receitasChef.php?success=3"); // Código 3 para edição
            exit();
        } else {
            // Modo adição
            ...
            $conn->commit();
            header("Location: receitasChef.php?success=1"); // Código 1 para adição
            exit();
        }
    } catch (Exception $e) {
        $conn->rollBack();
...
<body>
    <div class="container">
        ...
        <form method="POST" class="insert-bar">
            ...
            <?php if ($edicaoAtiva): ?>
                <a href="receitasChef.php" class="button cancelar-edicao">Cancelar</a>
            <?php endif; ?>
        </form>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="receitasChef.php?editar=<?php echo $receita['id']; ?>">Editar</a>
                            <a href="receitasChef.php?excluir=<?php echo $receita['id']; ?>" onclick="return confirm('Tem certeza que deseja excluir esta receita?')">Excluir</a>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
