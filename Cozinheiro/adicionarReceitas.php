...
if (isset($_POST['adicionar'])) {
    ...
    try {
        ...
        // Mantenha 'ingredientes_lista_texto' se ainda quiser usá-lo para algo (ex: resumo)
        // Caso contrário, remova-o daqui e da sua tabela de banco de dados
        $sql = "INSERT INTO receitas (nome_receita, data_criacao, id_categoria, modo_preparo, porcoes, tempo_preparo, dificuldade, descricao, ingredientes_lista_texto, id_funcionario)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        // O valor para ingredientes_lista_texto pode ser null, ou um resumo gerado, ou apenas removido se não for usar
        $stmt->execute([$nome, $data, $id_categoria, $modo_preparo, $porcoes, $tempo_preparo, $dificuldade, $descricao_geral, null, $id_funcionario]); 
